...
VivaldiAccInit(reload := false) {
    static vTabs := 0
    If (reload || !vTabs) {
        vTabs := AccMatchTextAll("ahk_exe vivaldi.exe",{"roletext":"push button","name":"Menu"},"path",,1,18)
...
VivaldiFocusTab(name) {
  static title := "ahk_exe vivaldi.exe"
  If !WinActive(title) {
    WinActivate, % title
    sleep 10
  }
  path := AccMatchTextAll(title,{name:name},"path",0,0,17)
  for _, p in StrSplit(Acc_Get("location",path,0,title),A_Space)
  ...
  if (x) {
    ...
    MouseGetPos, x2, y2
    Click, % (x +3) . A_Space . (y +3) ;. A_Space . "0"
...
JEE_ChromeAccInit(vValue)
{
    ...
    if !vTabs
        vTabs := AccMatchTextAll("ahk_exe chrome.exe",{"roletext":"page tab list"},"path",,1,20) . ".1"
	if (vValue = "U1")
...
AccMatchTextAll(title, matchList, get := "path", regex := 0, reload := 0, depthLimit := 0, getObj := 0) {
    static
    start := A_TickCount
